...
\begin{abstract}
We consider infinite-dimensional linear inverse problems 
of estimating $m$ from the observation model $y = \F m + \eta$. 
We study two questions: (i) Given a fixed prior, to what extent $m$ 
is learnable if we are free to choose $\F$? (ii) given 
a fixed prior and $\F$, which linear prediction functionals of $m$ 
are most learnable? 
\end{abstract}
