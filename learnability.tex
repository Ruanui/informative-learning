...
\runtitle{Some questions of learnability in linear inverse problems}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\author[A]{\fnms{Alen}~\snm{Alexanderian}\ead[label=e1]{first@somewhere.com}}
\and
\author[B]{\fnms{Ruanui}~\snm{Nicholson}\ead[label=e2]{second@somewhere.com}\orcid{0000-0000-0000-0000}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Addresses                                %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\runauthor{A. Alexanderian et al.}
\end{aug}
...
\end{aligned}
\end{equation}
with $\kappa>0$ up to us. It will be useful to let $\mathcal{L}=-\frac{d}{dx}(\kappa(x)\frac{d}{dx})$. Assume we measure in an interval (ball) of radius $r>0$ so that 

\[\B u:=\int_{x_0-r}^{x_0+r} u(x) \;dx\]

Then we can write the measurement (in terms of $m$) as~\cite{AlenBA,alexanderian2021optimal}

\[\B u= \int_{x_0-r}^{x_0+r}  \int_{0}^{1} G(s,x;\kappa)m(x) \;dx\;ds=  \int_{0}^{1} \int_{x_0-r}^{x_0+r} G(s,x;\kappa) \;ds\; m(x)\;dx=\langle g_\kappa,m\rangle,\]
...
we want to try to select $\kappa$ such that $g_\kappa=\phi_1$, that is pick $\kappa$ such that 
\[g_\kappa=\int_{x_0-r}^{x_0+r} G(s,x;\kappa)\;ds=\int_{x_0-r}^{x_0+r} G(x,s;\kappa)\;ds=\phi_1.\]
 {\color{red} Need to sort the normalisation!} Clearly $\phi_1$ is dependent on the choice of $\mathcal{C}$. As a concrete example, suppose $\mathcal{C}=\mathcal{A}^{-2}$ with $\mathcal{A}:=\alpha I -\Delta$ with homogeneous Dirichlet boundary conditions. Then $\phi_1=\sin(\pi x)$ and we want to set
 \[\int_{x_0-r}^{x_0+r} G(s,x;\kappa)\;ds=\sin(\pi x).\]
 
By definition we have
 \begin{align}
\mathcal{L}G(x,s)&=\delta(x-s)\quad \text{in } (0,1)\\
G(0,z)&=G(1,s)=0.
\end{align}
Then
 \begin{align}
\mathcal{L}g_\kappa&=\mathcal{L}\int_{x_0-r}^{x_0+r} G(s,x;\kappa)\;ds=\int_{x_0-r}^{x_0+r} \mathcal{L}G(s,x;\kappa)\;ds=\int_{x_0-r}^{x_0+r}\delta(x-s)\;ds=\chi([x_0-r,x_0+r])
\end{align}
Plugging in what we want gives
 \begin{align}
\mathcal{L}\phi_1&=\chi_{(x_0-r,x_0+r)}(x)
\end{align}
That is to say, we solve
 \begin{align}
-\frac{d}{dx}(\kappa(x)\frac{d}{dx}\phi_1)&=\chi_{(x_0-r,x_0+r)}(x)
\end{align}
for $\kappa$, giving
 \begin{align}
\kappa=\frac{C-\int_0^x\chi_{(x_0-r,x_0+r)}(q)\;dq}{\phi_1'}
\end{align}
